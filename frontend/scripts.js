document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        ...
        console.log(err);
        console.log("Admin data cannot be loaded now!");
    }
...
async function handleBranchChange(event) {
    ...
    try {
        const response = await fetch("http://localhost:3000/api/branch-change", {
            method: "POST",
            headers: {
...
async function loadAdminData() {
    ...
    try {
        const [requestsResponse, vacanciesResponse] = await Promise.all([
            ...
            fetch("http://localhost:3000/api/admin/vacancies"),
        ]);
        ...
        populateTable(studentTable, requests, "student");
...
async function deleteRequest(rollNo) {
    try {
        const response = await fetch(`http://localhost:3000/api/admin/delete-request/${rollNo}`, {
            method: "DELETE",
        });
